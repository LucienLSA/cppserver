...
class Channel {
private:
    EventLoop *loop; //每个文件描述符会被分发到一个Epoll类，用一个ep指针来指向
    int fd;
    ...
    std::function<void()> callback; // 事件回调函数，在 handleEvent() 时被调用。
public:
    Channel(EventLoop *_loop, int _fd); // 初始化时必须关联Epoll实例和文件描述符
    ~Channel();
    ...
    void handleEvent(); // 当 epoll_wait 检测到 fd 就绪时，由 EventLoop 调用
    void setCallback(std::function<void()>); // 绑定事件回调函数（如读/写处理逻辑）。
};